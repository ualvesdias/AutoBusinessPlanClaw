...
def test_obsidian_export_cleans_stale_managed_notes(tmp_path):
    config_path = tmp_path / "config.yaml"
    config_path.write_text(
        """
project:
...
""",
    ...
    stale_note = vault_dir / "03 Competition" / "Competitor - Stale.md"
    stale_note.parent.mkdir(parents=True, exist_ok=True)
    stale_note.write_text("old", encoding="utf-8")

    export_run_to_obsidian(run_dir, vault_dir)

    assert not stale_note.exists()
    assert (vault_dir / "03 Competition" / "Competition Matrix.md").exists()
